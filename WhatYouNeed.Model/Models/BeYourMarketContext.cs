...
using Appli.Model.Models.Mapping;
using Appli.Core.Migrations;
using Appli.Core;
...
{
    public partial class AppliContext : Repository.Pattern.Ef6.DataContext
    {
        static AppliContext()
        {
            // Check if migrate database to latest version automatically (using automatic migration)D:\Soft\DVP\DvpASY_Web2018\TrucsNTrocsMarket\src\Appli.Model\Models\AppliContext.cs
            ...
            if (AppliConfigurationManager.MigrateDatabaseToLatestVersion){
                Database.SetInitializer(new MigrateDatabaseToLatestVersion<AppliContext, ConfigurationInstall<Appli.Model.Models.AppliContext>>());
            }
            else { 
